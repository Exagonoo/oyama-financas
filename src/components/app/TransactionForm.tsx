...
export function TransactionForm({
  ...
  const pendingScopeResolve = useRef<((scope: "single" | "all") => void) | null>(null);
  ...
  const mutation = useMutation({
    mutationFn: async (scope: "single" | "all" = "single") => {
      const { data: userData } = await supabase.auth.getUser();
      const userId = userData.user?.id;
      if (!userId) throw new Error("Não autenticado");
      ...
      if (transaction) {
        ...
        if (scope === "all" && transaction.recurrence_group_id) {
  ...
  return (
    <>
            ...
            >
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  placeholder={
                    type === "transfer" ? "Ex.: Transferência" : "Ex.: Mercado da semana"
                  }
      ...
      <AlertDialog open={scopeDialogOpen} onOpenChange={setScopeDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Aplicar alteração a quem?</AlertDialogTitle>
            <AlertDialogDescription>
              Este lançamento faz parte de uma série recorrente. Deseja aplicar as alterações apenas
              a este ou a todos?
            </AlertDialogDescription>
          </AlertDialogHeader>
